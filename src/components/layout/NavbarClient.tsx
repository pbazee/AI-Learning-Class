...
const iconButtonClassName =
  "inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 text-slate-700 transition hover:border-primary-blue/25 hover:bg-primary-blue/5 hover:text-primary-blue dark:border-slate-800 dark:text-slate-200 dark:hover:bg-slate-900";
const mobileIconButtonClassName =
  "inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-slate-200 text-slate-700 dark:border-slate-800 dark:text-slate-200";
...
export function NavbarClient({
  ...
  useEffect(() => {
    ...
    fetch("/api/settings?keys=siteName,logoUrl")
      .then((response) => response.json())
      .then((payload) =>
        setBranding({
          siteName: payload?.siteName || DEFAULT_SITE_NAME,
    ...
    <Dialog.Root open={menuOpen} onOpenChange={setMenuOpen}>
      <nav ref={navRef} className="sticky top-[var(--announcement-height)] z-[90] border-b border-slate-200/80 bg-white/95 shadow-[0_18px_50px_-38px_rgba(15,23,42,0.3)] backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/95">
        <div className="header-frame">
          {/* Desktop Nav */}
          <div className="hidden min-h-[56px] items-center gap-3 lg:flex">
            <Link href="/" className="flex shrink-0 items-center gap-3">
              <SiteLogo
                siteName={branding.siteName}
                logoUrl={branding.logoUrl || undefined}
                textClassName="text-[15px] text-slate-950 dark:text-white"
              />
            </Link>
            ...
            <div className="ml-auto flex items-center gap-2">
              <Link href="/wishlist" className={iconButtonClassName}><Heart className="h-[18px] w-[18px]" /></Link>
            ...
            <div className="flex items-center gap-2">
              <Link href="/" className="flex shrink-0 items-center gap-2">
                <SiteLogo
                  ...
                  logoUrl={branding.logoUrl || undefined}
                  compact
                  textClassName="hidden text-[11px] text-slate-950 dark:text-white sm:inline"
                />
              </Link>
              ...
              {mounted ? <button type="button" onClick={() => setTheme(resolvedTheme === "dark" ? "light" : "dark")} className={mobileIconButtonClassName}>{resolvedTheme === "dark" ? <SunMedium className="h-4 w-4" /> : <MoonStar className="h-4 w-4" />}</button> : null}
              <Dialog.Trigger asChild>
                <button type="button" className={mobileIconButtonClassName} aria-label="Open menu">
