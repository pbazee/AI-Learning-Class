          ...
          {posts.map((post, i) => (
            ...
            >
              <Link href={`/blog/${post.slug}`} className="group block h-full">
                <div className="surface-card flex h-full flex-col overflow-hidden transition-all hover:-translate-y-1 hover:border-blue-300">
                  <div className="relative aspect-video overflow-hidden">
                    ...
                    <div className="absolute bottom-3 left-3 flex flex-wrap gap-2">
                      {post.tags.slice(0, 2).map((tag) => (
                        <span
                          key={tag}
                          className="rounded-full bg-white/90 px-2.5 py-1 text-[11px] font-semibold text-blue-700 shadow-sm"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    <h3 className="mb-2 line-clamp-2 text-base font-bold leading-snug text-foreground transition-colors group-hover:text-blue-700">
                      {post.title}
                    </h3>
                    {post.excerpt && (
