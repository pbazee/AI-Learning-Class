...
export function BlogSharePopover({
  canonicalUrl,
  title,
}: {
  canonicalUrl: string;
  ...
  const shareLinks = useMemo(() => {
    ...
    const whatsappText = encodeURIComponent(`Check this out: ${title} ${canonicalUrl}`);
  ...
  return (
    <div ref={containerRef} className="relative ml-auto">
      ...
      {open ? (
        <div className="absolute right-0 top-auto z-[120] mt-0 w-72 max-w-[calc(100vw-16px)] rounded-2xl border border-border bg-card p-3 shadow-2xl max-sm:bottom-full max-sm:mb-3 sm:top-full sm:mt-3">
