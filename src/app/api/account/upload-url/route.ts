...
export async function POST(request: Request) {
  try {
    ...
    if (!user) {
      return NextResponse.json({ error: "Please sign in first." }, { status: 401, headers: CORS_HEADERS });
    }
    ...
    const folder = sanitizeFolder(
      typeof body?.folder === "string" && body.folder.trim() ? body.folder : "avatars/learners"
    );
    ...
    const key = `${folder}/${user.id}/${Date.now()}-${crypto.randomUUID()}-${safeFileName}`;
    ...
    console.error("[account.upload-url] Unable to create a presigned upload URL.", error);
    return NextResponse.json(
